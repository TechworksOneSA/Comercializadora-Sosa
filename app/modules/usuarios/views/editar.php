    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form method="POST" action="<?= url('/admin/usuarios/actualizar/' . $usuario['id']) ?>" class="space-y-6">
            ...
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-6">
                ...
                <div class="flex flex-col items-center mb-4">
                    <div id="photoPreview" class="w-32 h-32 bg-gray-200 rounded-full flex items-center justify-center border-2 border-gray-300 mb-3">
                        <?php if (!empty($usuario['foto'])): ?>
                            <img src="<?= htmlspecialchars($usuario['foto']) ?>" class="w-full h-full object-cover rounded-full" alt="Foto del usuario">
                        <?php else: ?>
                ...
                <!-- Botón para limpiar -->
                <button type="button" id="clearPhotoBtn" class="w-full bg-red-100 hover:bg-red-200 text-red-700 px-4 py-2 rounded-lg transition <?= empty($usuario['foto']) ? 'hidden' : '' ?>">
                    🗑️ Limpiar Foto
                </button>
            </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        `;
            photoDataInput.value = ''; // Esto es importante: campo vacío pero presente
